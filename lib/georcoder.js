...
Georcoder.prototype.get = function (method,provider,data,params) {
  ...
  if(method === 'reverse') {
    ...
    if(typeof provider.arcgisauth === 'object') {
      return new Promise(_.bind(function(resolve,reject){
        provider.reverse(data,params).then(_.bind(function(response){
          resolve({
            source: 'arcgis',
            type: 'extend',
            data: this.formatter('arcgis',method,response,{data: data, params: params}).properties
          });
        },this)).
        catch(function(error){
          reject(error);
...
Georcoder.prototype.geojson = function (data,properties) {
  ...
  return {
      ...
      geometry: {
        type: 'Point',
        coordinates: [
          Number(coords[0]),  //  longitude
          Number(coords[1])   //  latitude
        ]
      },
      properties: properties
  };
